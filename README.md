...
## Почтовая отправка
По умолчанию сервер использует системный `sendmail`. Для SMTP-настройки перед запуском установите переменные окружения:
...
- `SENDMAIL_PATH` — путь к `sendmail`, если он отличается от `/usr/sbin/sendmail`.
