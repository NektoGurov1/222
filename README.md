...
3. Запустите сервер:
   ...
   python app.py
   ```
4. Откройте `http://localhost:9090/` — увидите лендинг. Страница входа находится на `http://localhost:9090/login`, панель управления — на `http://localhost:9090/admin`.
...
## Почтовая отправка
По умолчанию сервер использует системный `sendmail`. В Docker-образе (см. `Dockerfile`) пакет `sendmail` устанавливается при сборке, поэтому дополнительных действий не требуется: форма заявки сможет отправлять письма сразу после развёртывания контейнера. Для SMTP-настройки перед запуском установите переменные окружения:
- `SMTP_HOST` — адрес почтового сервера.
- `SMTP_PORT` — порт (по умолчанию 587).
- `SMTP_SECURE` — `true`/`false` (включить ли TLS).
